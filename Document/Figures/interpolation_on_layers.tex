...
\begin{document}
	\begin{tikzpicture}[scale = 0.35, baseline]
		...
		% First layer axis
		\draw[name path = layer1, ->] (-1, 3) --(11, 3) node[right] {Layer 1};
		
		% Second layer axis
		\draw[name path = layer2, ->] (-1, -1) --(11, -1) node[right] {Layer 2};
		...
		\foreach \i in {1,...,3} 
			\fill ({1.5 + (\i - 1) * 3}, 3) circle[radius = 0.15];	
		
		% Pixel centers on second layer	
		\foreach \i in {1,...,3} 
			\fill ({1.5 + (\i - 1) * 3}, -1) circle[radius = 0.15];	
		...
		\fill[red] (anchor) circle[radius = 0.15];
		...
		\draw[name intersections={of=layer1 and ray, total=\t}, blue, |-|]
			(intersection-1) ++(0, -1) -- node[below, yshift = -1.5] {$1 - \alpha$} (7.5, 2);
		
		\draw[name intersections={of=layer1 and ray, total=\t}, blue, |-|]
			(intersection-1) ++(0, 1) -- node[above, yshift = 1.5] {$\alpha$} (4.5, 4);
			
		% Interpolation markers and labels on second layer
		\draw[blue, |-|]
			(anchor) ++(0, 1) -- node[above, yshift = 1.5] {$\beta$} (1.5, 0);
				
		\draw[blue, |-|]
			(anchor) ++(0, -1) -- node[below, yshift = -1.5] {$1 - \beta$} (4.5, -2);
