...
For solving the least squares problem in~\ref{eq:minimize_norm}, two iterative solvers are compared in terms of reconstruction error and runtime.
...
To summarize the official documentation, the algorithm performs the following steps.

The function to minimize is $f(\alpha) = \lVert P \alpha + \bar{L} \rVert^2$.
Let $\alpha_{i - 1}$ be the result of the previous iteration.
The goal for the current iteration is to find $\alpha_i$ in the neighborhood of $\alpha_{i - 1}$ for which $f$ improves:
\begin{enumerate}
	...
	\item 	If $f(\alpha_{i - 1} + s) < f(\alpha_{i - 1})$, set $\alpha_i \coloneqq \alpha_{i - 1} + s$ and the iteration is completed.
