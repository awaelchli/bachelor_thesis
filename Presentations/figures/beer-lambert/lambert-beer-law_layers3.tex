...
%%%%%%%%%%%%%%%%%%%%
\node[anchor    = north,
      inner sep = 1ex] at (current bounding box.south){$\dfrac{I}{I_0}=\text{exp} \displaystyle \left(-\int_{\mathcal{R}} \mu (r) \text{d}r \right) = \displaystyle \prod_{i} t^{(i)} = \text{exp} \left( \displaystyle - \sum_{i} a^{(i)} \right)$};
